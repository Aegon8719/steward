...
### 2026-09-10（core-engine 查询 needle 小写归一）

- 补 `core-engine::Engine::query` 的 needle 小写归一，与 2026-08-27 插件路由的同一条约定对齐。此前混合大小写查询在 ASCII haystack 上会触发 nucleo 的 `should have been caught by prefilter` 断言（release 为 `panic = "abort"`，直接终止进程），在含中文的路径 haystack 上则静默不命中；已加回归测试覆盖。

### 2026-09-11（Windows 文件对话框目录快切）

- 交互：受支持的打开／另存为对话框出现时，Steward 搜索框自动贴出在对话框正下方，宽度与对话框一致、水平对齐；首次出现时自动接管键盘焦点，预填并选中最近活动的资源管理器目录，用户可直接输入或修改查询，选择目录后按 Enter 导航原对话框；Esc 取消并返回原对话框。完成、取消或切走后恢复进入前的启动器查询。
- 归属：能力放在 `crates/app/src/quick_switch/`，复用搜索输入、结果列表和本地化，不经插件运行时。目录模式只返回目录，避免把应用或插件命令误当成对话框导航目标。
- 目录来源：COM 工作线程每 250 ms 读取前台资源管理器的可见活动视图，仅记录文件系统目录；去重历史最多 50 项，仅保留在内存。查询复用 `nucleo` 匹配历史和常用目录；绝对路径输入提供当前目录及子目录候选，不建立全盘索引。COM 与文件系统工作移出 GPUI 前台线程，查询结果按代次丢弃过期回复。
- 对话框边界：仅识别含 Shell 文件视图和地址栏的现代标准 Windows 文件对话框，保存其窗口、进程、线程与地址栏身份；导航前重新验证并恢复焦点。通过 Alt+D 聚焦地址栏，仅向已验证的地址编辑框写入目录并提交导航，不操作文件名、打开／保存按钮或剪贴板。自定义／旧式对话框不接管。
- 焦点与生命周期：新对话框首次出现时搜索框接管键盘（`SW_SHOW` + `AttachThreadInput`/`SetForegroundWindow`），系统拒绝焦点转移（如管理员权限对话框）时退回点击输入提示。用户点击对话框、按 Esc 或完成导航后，搜索框保持贴附但不抢焦点，点击搜索框重新接管输入；对话框关闭或切走时自动隐藏，重新获得焦点时再次贴出。
- 窗口位置：搜索框钉在对话框下沿（左对齐、等宽），对话框移动或调整大小时同步更新；结果列表在下方展开，不为避让屏幕边沿上移或缩窄。普通启动器唤起仍按原居中位置与设计宽度（760px）。
- 验证边界：实现不等于跨应用兼容性已验证。真实打开／保存对话框、Windows 11 多标签页、UNC 与跨权限进程仍需人工验收，操作步骤及检查表见 [目录快切](quick-switch.md)。
